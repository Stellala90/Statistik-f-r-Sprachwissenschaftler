...
# Fall?
two.sample <- t.test(rt$RT~rt$subj, var.equal=TRUE)
...
logrt1 <- rt[rt$subj == "1","logRT"]
logrt2 <- rt[rt$subj == "2","logRT"]
log_F <- var.test(logrt1,logrt2)
print(log_F)
...
log_levene <- leveneTest(logRT_gesamt$logRT ~ logRT_gesamt$subj)
print(log_levene)
