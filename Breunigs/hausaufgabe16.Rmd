...
# Analyse
Haben Gewicht (carat) und Farbe (color) einen Einfluss auf den Preis?

```{r}
anova.1 <- aov(price ~ carat*color, data=diamonds)
summary(anova.1)
```
...
ggplot(diamonds, aes(x=carat, y=price, color=color)) + geom_point() + facet_wrap(~color)
```
 Das Merkmal der Farbe ist an der Graphik nicht besonders gut zu erkennen, deswegen noch eine Graphik im "Violin"-Format:

```{r}
ggplot(diamonds) + geom_violin(aes(x=color, y=price, color=color, fill = color),alpha=0.3)
```
Das Gewicht hat einen größeren Einfluss auf den Preis als die Farbe.
Je schwerer der Diamant, desto teurer ist er. Der Einfluss der Farben auf den Preis ist weniger eindeutig. Scheinbar sind Diamanten der Farbe J besonders teuer, die der Farbe E besonders günstig.
Die Graphiken veranschaulichen dieses Ergebnis.
