...
# Analyse
Welchen Zusammenhang haben Karat und Klarheit/Farbe von Diamanten?

ggplot(diamonds, aes(x=carat, y=clarity, color=color))+ geom_point(alpha=.5) 

Die Grafik lässt vermuten: Je mehr Karat, desto schlechter die Klarheit.
Ist das denn wirklich so? Ich schaue mir die Daten an:

summary(lm(carat~clarity,data=diamonds))
 
Der Test ergibt einen p-Wert von <2e-16, womit der beschriebene Zusammenhang von Karat und Klarkeit bei einem vorgegebenen Signifikanzniveau von 5 % signifikant ist.


Einfluss der kategorialen Variable Farbe?
Anhand der Darstellung der vorherigen Grafik ist nur schwer ein Zusammenhang erkennbar. 
Versuch einer besseren Dastellung:

ggplot(diamonds, aes(x=carat, y=color))+ geom_point(alpha=.5)

Es scheint, dass je mehr Karat, desto schlechter die Farbe (z.B ~2,75 Karat entspricht einer Farbe zwischen H und J, wobei ~2,0 Karat eher einer Farbe zwischen F und D entspricht). 
Ist der Einfluss von Farbe auf Karat signifikant? 

diamonds$color.numeric <- as.numeric(diamonds$color)
summary(lm(carat~color.numeric,data=diamonds))

Auch der Einfluss von Farbe auf Karat ist mit einem p-Wert von <2.2e-16 signifikant.
